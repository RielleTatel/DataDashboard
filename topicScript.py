#!/usr/bin/env python3
"""
Topic Analysis Script for Misinformation Articles Dataset
========================================================

This script analyzes the TEXT column of your dataset and generates topic categories
for each article using advanced NLP techniques including:
...
try:
    ...
    print("Warning: BERT libraries not available. Install with: pip install transformers sentence-transformers torch")
...
class TopicAnalyzer:
    """
    Advanced topic analyzer for misinformation articles using multiple NLP techniques.
    """
    ...
    def extract_keywords_tfidf(self, texts: List[str], max_features: int = 1000) -> Dict:
        """
        ...
        Returns:
            Dict: Dictionary with document keywords
        """
        ...
        for i, text in enumerate(texts):
            if text.strip():  # Only process non-empty texts
                ...
                top_indices = doc_scores.argsort()[-10:][::-1]  # Top 10 keywords
                keywords = [feature_names[idx] for idx in top_indices if doc_scores[idx] > 0]
                document_keywords[i] = keywords
        
        return document_keywords
    
    def classify_with_keywords(self, text: str) -> Tuple[str, float]:
        """
        ...
        Returns:
            Tuple[str, float]: (topic, confidence_score)
        """
    ...
    def perform_lda_analysis(self, texts: List[str], n_topics: int = 10) -> Dict:
        """
        ...
        Returns:
            Dict: LDA results with topic distributions
        """
    ...
    def perform_nmf_analysis(self, texts: List[str], n_topics: int = 10) -> Dict:
        """
        Perform Non-negative Matrix Factorization analysis.
        ...
        Returns:
            Dict: NMF results with topic distributions
        """
        logger.info(f"Performing NMF analysis with {n_topics} topics...")
        ...
        for topic_idx, topic in enumerate(self.nmf_model.components_):
            top_keywords = [feature_names[i] for i in topic.argsort()[-10:][::-1]]
            topic_keywords[f'nmf_topic_{topic_idx}'] = top_keywords
        
        self.topic_keywords.update(topic_keywords)
        
        # Get dominant topic for each document
        document_topics = {}
        for i, doc_topics in enumerate(nmf_output):
            if texts[i].strip():  # Only process non-empty texts
                dominant_topic = doc_topics.argmax()
                confidence = doc_topics[dominant_topic] / doc_topics.sum() if doc_topics.sum() > 0 else 0
                document_topics[i] = {
                    'topic': f'nmf_topic_{dominant_topic}',
                    'confidence': confidence,
                    'all_topics': doc_topics.tolist()
                }
    ...
    def perform_bert_analysis(self, texts: List[str], n_topics: int = 10) -> Dict:
        """
        Perform BERT-based topic analysis using sentence embeddings.
        ...
        Returns:
            Dict: BERT clustering results
        """
        if not BERT_AVAILABLE:
            logger.warning("BERT not available, skipping BERT analysis")
            return {}
        
        logger.info("Performing BERT-based topic analysis...")
        ...
        try:
            ...
            confidences = 1 / (1 + distances[np.arange(len(cluster_labels)), cluster_labels])
            
            # Map back to original indices
            document_topics = {}
            for idx, (orig_idx, label, conf) in enumerate(zip(valid_indices, cluster_labels, confidences)):
                document_topics[orig_idx] = {
                    'topic': f'bert_cluster_{label}',
                    'confidence': conf,
    ...
    def analyze_dataset(self, file_path: str, output_path: str = None) -> pd.DataFrame:
        """
        ...
        Returns:
            pd.DataFrame: DataFrame with original data and topic analysis results
        """
        ...
        results['lda_analysis'] = lda_results
        
        # 4. NMF analysis
        logger.info("Performing NMF topic modeling...")
        nmf_results = self.perform_nmf_analysis(texts, n_topics=8)
        results['nmf_analysis'] = nmf_results
        
        # 5. BERT analysis (if available)
        if BERT_AVAILABLE:
            logger.info("Performing BERT-based analysis...")
            bert_results = self.perform_bert_analysis(texts, n_topics=8)
            results['bert_analysis'] = bert_results
        
        # Combine results into DataFrame
        logger.info("Combining analysis results...")
        
        # Add keyword analysis results
        df['KEYWORD_TOPIC'] = [r['topic'] for r in results['keyword_analysis']]
        df['KEYWORD_CONFIDENCE'] = [r['confidence'] for r in results['keyword_analysis']]
        
        # Add LDA results
        df['LDA_TOPIC'] = ''
        df['LDA_CONFIDENCE'] = 0.0
        for idx, result in results['lda_analysis'].items():
            df.loc[idx, 'LDA_TOPIC'] = result['topic']
            df.loc[idx, 'LDA_CONFIDENCE'] = result['confidence']
        
        # Add NMF results
        df['NMF_TOPIC'] = ''
        df['NMF_CONFIDENCE'] = 0.0
        for idx, result in results['nmf_analysis'].items():
            df.loc[idx, 'NMF_TOPIC'] = result['topic']
            df.loc[idx, 'NMF_CONFIDENCE'] = result['confidence']
        
        # Add BERT results if available
        if BERT_AVAILABLE and results.get('bert_analysis'):
            df['BERT_TOPIC'] = ''
            df['BERT_CONFIDENCE'] = 0.0
            for idx, result in results['bert_analysis'].items():
                df.loc[idx, 'BERT_TOPIC'] = result['topic']
                df.loc[idx, 'BERT_CONFIDENCE'] = result['confidence']
        
        # Add TF-IDF keywords
        df['TOP_KEYWORDS'] = ''
        for idx, keywords in results['tfidf_keywords'].items():
            df.loc[idx, 'TOP_KEYWORDS'] = ', '.join(keywords[:5])  # Top 5 keywords
        
        # Create final topic classification
        df['FINAL_TOPIC'] = df['KEYWORD_TOPIC']
        df['FINAL_CONFIDENCE'] = df['KEYWORD_CONFIDENCE']
        
        # Use LDA or NMF if keyword confidence is low
    ...
    def _generate_summary(self, df: pd.DataFrame):
        ...
        print(f"  Average NMF confidence: {df['NMF_CONFIDENCE'].mean():.3f}")
        
        if 'BERT_CONFIDENCE' in df.columns:
            print(f"  Average BERT confidence: {df['BERT_CONFIDENCE'].mean():.3f}")
    ...
    def create_visualizations(self, df: pd.DataFrame, output_dir: str = "topic_analysis_plots"):
        """
        ...
        # 3. Topic comparison bar chart
        plt.figure(figsize=(15, 8))
        comparison_data = pd.DataFrame({
            'Keyword': df['KEYWORD_TOPIC'].value_counts().head(8),
            'LDA': df['LDA_TOPIC'].value_counts().head(8),
            'NMF': df['NMF_TOPIC'].value_counts().head(8)
        })
        
        comparison_data.plot(kind='bar', figsize=(15, 8))
        plt.title('Topic Distribution Comparison Across Methods', fontsize=16, fontweight='bold')
        plt.xlabel('Topics')
        plt.ylabel('Number of Articles')
        plt.xticks(rotation=45, ha='right')
        plt.legend()
        plt.tight_layout()
        plt.savefig(f"{output_dir}/topic_comparison.png", dpi=300, bbox_inches='tight')
        plt.close()
        
        # 4. Word cloud for most common keywords
        if 'TOP_KEYWORDS' in df.columns:
            all_keywords = ' '.join(df['TOP_KEYWORDS'].dropna())
            if all_keywords.strip():
                wordcloud = WordCloud(width=800, height=400, background_color='white', 
                                    max_words=100, colormap='viridis').generate(all_keywords)
                
                plt.figure(figsize=(16, 8))
                plt.imshow(wordcloud, interpolation='bilinear')
                plt.axis('off')
                plt.title('Most Common Keywords in Dataset', fontsize=16, fontweight='bold')
                plt.tight_layout()
                plt.savefig(f"{output_dir}/keyword_wordcloud.png", dpi=300, bbox_inches='tight')
                plt.close()
...
def main():
    ...
    input_files = [
        "Source file/Fake.csv",
        "Source file/True.csv",
    ]
